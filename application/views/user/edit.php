...
<form id="editUser" method="post" action="<?= $Url::link("admin/adminusers/edit&id=" . $_GET['id'])?>">
    ...
    <h5>Введите пароль (оставьте пустым, если не нужно менять)</h5>
    <input type="password" name="pass" placeholder="новый пароль" value=""><br>
    
    <h5>Выберите роль пользователя</h5>
    <select name="role">
        <option value="admin" <?= ($viewAdminusers->role == 'admin') ? 'selected' : '' ?>>Администратор</option>
        <option value="auth_user" <?= ($viewAdminusers->role == 'auth_user') ? 'selected' : '' ?>>Авторизованный пользователь</option>
    </select><br>
